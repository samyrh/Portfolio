...
const Navbar = () => {
  ...
  // Scroll spy config
  const scrollOffsetPx = 88; // adjust to your navbar height
  ...
  const handleDownload = (lang) => {
    ...
    setCvOpen(false);
    showToastNotification(lang === 'en' ? 'English CV downloaded successfully.' : 'CV Français téléchargé avec succès.', 'success');
  };
  ...
  return (
      <nav
          className={`
        w-full fixed top-0 z-20 py-5 transition-colors duration-300
        ${scrolled ? "bg-primary/90 backdrop-blur-sm shadow-lg" : "bg-transparent"}
      `}
          role="navigation"
          aria-label="Primary"
      >
        {/* relative container so center nav can be absolutely centered */}
        <div className="relative w-full min-w-0 grid grid-cols-[auto_1fr_auto] items-center px-4 sm:px-8 md:px-[75px] lg:px-8">
          {/* LEFT: logo + name (kept in the same position) */}
          <Link
              to="/"
          ...
          >
            <img src={s} alt="Logo" className="w-10 h-10 sm:w-11 sm:h-11 md:w-12 md:h-12 object-contain" />
            <span className="text-white font-extrabold leading-none tracking-tight whitespace-nowrap text-[clamp(22px,3.6vw,44px)]">
            Sami Rhalim
          </span>
          </Link>

          {/* CENTER: desktop nav — perfectly centered */}
          <ul className="hidden sm:flex justify-self-center items-center gap-4 md:gap-6 lg:gap-8 xl:gap-10 px-2">
            {navLinks
                .filter((nav) => nav.id !== "contact")
                .map((nav) => (
                <li
                    key={nav.id}
                        className={`group relative cursor-pointer transition-colors duration-300 ${active === nav.title ? "text-white" : "text-secondary hover:text-white"}`}
                >
                      <a href={`#${nav.id}`} onClick={(e) => handleNavClick(nav.id, nav.title)(e)} className="text-[14px] md:text-[15px] lg:text-[16px] font-medium whitespace-nowrap">
                        {nav.title}
                      </a>
                      <span className={`absolute left-0 -bottom-1 h-[2px] w-full rounded bg-white origin-left transition-transform duration-300 ${active === nav.title ? "scale-x-100" : "scale-x-0 group-hover:scale-x-100"}`} />
                    </li>
                ))}
          </ul>

          {/* RIGHT: desktop CTA (with hover shining) */}
          <div className="hidden sm:flex ml-auto items-center gap-3 md:gap-4 lg:gap-5">
            {/* Hire Me */}
            <a
                href="#contact"
                className="group relative inline-flex items-center justify-center overflow-hidden rounded-full px-5 h-10 text-sm md:text-[15px] font-medium text-white tracking-wide shadow-md transition-all duration-300"
            >
      ...
      <span className="absolute inset-0 rounded-full p-[2px] [background:conic-gradient(from_180deg,theme(colors.fuchsia.500),theme(colors.violet.500),theme(colors.indigo.500),theme(colors.fuchsia.500))] opacity-70 blur-[1px] group-hover:animate-[spin_3s_linear_infinite] [animation-direction:reverse]" />
          ...
          {/* RIGHT: mobile menu button (shows on small screens) */}
              ...
              {toggle && (
                  <>
                    ...
                    <motion.div
                        ...
                        className="p-5 black-gradient fixed top-20 right-0 mx-4 my-2 min-w-[72%] max-w-[420px] z-40 rounded-2xl border border-white/10 shadow-2xl"
                    >
                      <ul className="flex flex-col gap-2 w-full">
                        ...
                        key={nav.id}
                            ...
                            >
                              <button
                                  className="w-full text-left text-[16px] font-medium cursor-pointer text-secondary hover:text-white px-3 py-2 rounded-xl hover:bg-white/5 transition-colors"
                ...
                <li>
                  <button
                              ...
                              className="relative inline-flex w-full items-center justify-center overflow-hidden rounded-full px-7 py-3 text-[15px] font-semibold text-white tracking-wide shadow-lg transition-all duration-300"
                    ...
                    <span className="relative z-10">Download CV</span>
                        ...
                        <li>
                          <a
                              ...
                              target="_blank"
                              rel="noopener noreferrer"
                              className="group relative inline-flex w-full items-center justify-center overflow-hidden rounded-full px-7 py-3 text-[15px] font-semibold text-white tracking-wide shadow-lg transition-all duration-300"
                          ...
                          >
                            ...
                            <span className="relative z-10 flex items-center justify-center gap-2">
                              <img src={github} alt="" className="w-6 h-6 object-contain" aria-hidden="true" />
                        ...
                        <li>
                          ...
                          >
                            ...
                            <span className="absolute inset-0 rounded-full [background:repeating-linear-gradient(135deg,rgba(255,255,255,0.08)_0px,rgba(255,255,255,0.08)_6px,transparent_6px,transparent_12px)] [mask-image:linear-gradient(to_bottom,transparent,black_20%,black_80%,transparent)] animate-[pulse_2.5s_ease-in-out_infinite]" />
                            <span className="relative z-10 flex items-center justify-center gap-2 leading-none scale-110">
                              <img src={linkedin} alt="" className="w-5 h-5 object-contain scale-150" aria-hidden="true" />
          ...
          {cvOpen && (
              <div className="fixed inset-0 z-[60] flex items-center justify-center p-6" role="dialog" aria-modal="true">
                ...
                <div className="relative z-[61] w-full max-w-2xl rounded-3xl overflow-hidden">
                  <div className="relative bg-gradient-to-b from-slate-900/90 to-slate-950/90 p-8 ring-1 ring-white/10">
                    ...
                    <h3 className="text-white text-2xl font-extrabold tracking-tight">Download CV</h3>
                    <p className="mt-2 text-sm text-white/70">Select your preferred language.</p>

                    <div className="mt-7 grid grid-cols-1 md:grid-cols-2 gap-4">
                      <button onClick={() => handleDownload('en')} className="group relative inline-flex items-center justify-center rounded-full px-6 py-4 text-white font-semibold ring-1 ring-white/10 overflow-hidden shadow-lg">
                        ...
                        <span className="absolute -inset-[3px] rounded-full">
                          ...
                          <span className="absolute inset-0 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 [background:
radial-gradient(120px_90px_at_25%_30%,rgba(255,255,255,0.28)_0%,transparent_60%),
radial-gradient(140px_110px_at_55%_20%,rgba(255,255,255,0.18)_0%,transparent_60%),
radial-gradient(110px_100px_at_75%_55%,rgba(255,255,255,0.22)_0%,transparent_60%),
radial-gradient(90px_80px_at_35%_70%,rgba(255,255,255,0.16)_0%,transparent_60%)]
] blur-[3px] [animation-play-state:paused] group-hover:animate-[spin_5s_linear_infinite]" />
                        </span>
                        <span className="relative">Download English CV</span>
                      </button>

                      <button onClick={() => handleDownload('fr')} className="group relative inline-flex items-center justify-center rounded-full px-6 py-4 text-white font-semibold ring-1 ring-white/10 overflow-hidden shadow-lg">
                        ...
                        <span className="absolute -inset-[3px] rounded-full">
                          ...
                          <span className="absolute inset-0 rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-300 [background:
radial-gradient(120px_90px_at_70%_30%,rgba(255,255,255,0.28)_0%,transparent_60%),
radial-gradient(140px_110px_at_45%_65%,rgba(255,255,255,0.18)_0%,transparent_60%),
radial-gradient(110px_100px_at_25%_55%,rgba(255,255,255,0.22)_0%,transparent_60%),
radial-gradient(90px_80px_at_65%_75%,rgba(255,255,255,0.16)_0%,transparent_60%)]
] blur-[3px] [animation-play-state:paused] group-hover:animate-[spin_5.4s_linear_infinite]" />
                        </span>
                        <span className="relative">Télécharger CV Français</span>
                      </button>
                    </div>

                    <button onClick={() => setCvOpen(false)} className="mt-8 w-full rounded-2xl bg-white/10 hover:bg-white/15 text-white py-3 font-semibold transition-colors">Close</button>
                  </div>
                </div>
              </div>
          )}

          {/* Toast Notification (matches Contact style) */}
          <AnimatePresence>
            {showToast && (
                <motion.div
                    ...
                    <div className="flex-1 overflow-hidden">
                      ...
                      <div className="mt-4 h-2 rounded-full bg-white/10 overflow-hidden">
                        <motion.div
                            ...
                            className={`h-full rounded-full ${toastType === "success" ? "bg-green-400" : "bg-red-400"}`}
                        />
                      </div>
                    </div>
                  </div>
